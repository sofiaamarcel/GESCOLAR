...
try
{
    ...
    if(isset($_REQUEST['cadastrar']))
    {
        ...
        echo "Matricula realixada com sucesso!";
    }
}catch(Exception $e) {
    echo $e->getMessage();
} 
?>  
<link href="css/estilos.css" type="text/css" rel="stylesheel" />

<?php include_once 'incluides/cabecalho.php' ?>

<div>
<fieldset>
    <legend>Nova Matricula</legend>
    <form action="cadastro_matricula.php?cadastrar-true" methood="post">
      <label>
      <select name ="id_turma">
      <?php while($turma = $stmt_turmas->fetchObject()): ?>
      <option value ="<?=$stmt_turma->id ?>"> <?=$turma->descricao ?> </option>
      <?php endwhile ?>
      </select>
    </label>
    <label>
